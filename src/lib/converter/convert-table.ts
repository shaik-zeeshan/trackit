import justify from "justify-text";
import { decode } from "html-entities";

const table_to_markdown = {
  max_width: 96,
  ...
  convert: function (table: string) {
    let result = "\n";

    let caption = table.match(/<caption[^>]*>((?:.|\n)*)<\/caption>/i);
    if (caption) result += this.clean(caption[1]) + "\n\n";
...
export default table_to_markdown;
